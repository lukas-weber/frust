...
static cmplx dispOp(int n, int l, double g) {
	...
	for(int a = 0; a <= std::min(n,l); a++) {
		cmplx term = std::pow(1i * g, n + l - 2*a)*exp(0.5*lgamma(1+l)+ 0.5*lgamma(1+n)-lgamma(1+a)-lgamma(1+n-a)-lgamma(1+l-a));
		sum += term;
	}
	return sum;
}
...
double J(int n, int m, double omega, double g, double tolerance = 1e-12) {
	...
	for(int l = 0; l < std::max(n,m)+2 || fabs(term) > tolerance; l++) {
		term = (dispOp(n,l,g) * std::conj(dispOp(m,l,g))).real() * (1.0/(1.0+omega*(l-n)) + 1.0/(1.0+omega*(l-m)));
		sum += term;
	}
