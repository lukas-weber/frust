...
parser = argparse.ArgumentParser(
    description="Compare ed results (generated by ed.py) to mc results (generated by isingsse). Both from the same jobfile, obviously."
)
...
parser.add_argument(
    "-o", "--observables", type=str, help="Regex to filter observables to be compared."
)
parser.add_argument(
    "-c", "--cli", action="store_true", help="cli-only mode. No plots are shown."
)
parser.add_argument(
    "-p",
    "--confidence",
    type=float,
    default=1e-4,
    help="confidence interval for the χ² test to fail.",
)
parser.add_argument(
    "-t",
    "--tolerance",
    type=float,
    default=1e-9,
    help="absolute tolerance in case there is no statistical error.",
)
...
if args.observables:
    obs_names = list(
        filter(lambda x: re.fullmatch(args.observables, x) != None, obs_names)
    )
...
def compare(obs_name, Ts, mean, error, edobs):
    ...
    smallerror = np.logical_and(
        mean - edobs < args.tolerance, error < args.tolerance
    )  # roundoff error regime
    ...
    if np.any(smallerror):
        print(
            "WARNING: some values have errors smaller than machine precision. Ergodicity?"
        )
    if len(maskmean) > min(len(mean), 3):
        ...
        print(
            "{} χ²/dof = {:.2g}, p = {:.3g}".format(obs_name, chisq / len(maskmean), p)
        )
    ...
    if not args.cli:
        ...
        plt.grid(True)
        plt.errorbar(
            Ts[mask], maskmean - edobs[mask], maskerror, fmt=".", label="ED-MC"
        )
        if np.any(smallerror):
            plt.plot(
                Ts[smallerror],
                mean[smallerror] - edobs[smallerror],
                ".",
                label="ED-MC (small error)",
            )
        plt.xlabel("T")
        plt.show()
