...
parser = argparse.ArgumentParser(description = 'Benchmark a job by evaluating a Monte Carlo efficiency measure: error at constant time')
parser.add_argument('jobfile', type=str, help='job to run')

args = parser.parse_args()

subprocess.run(['loadl', 'run', '--restart', '--single', args.jobfile], check=True, capture_output=True)
...
for obsname in ["SignEnergy", "Sign", "SignStagXStagYMag2"]:
    ...
    stime_error = obs.error/np.abs(obs.mean)*(obs.rebinning_bin_length*obs.rebinning_bin_count*(stime.mean+mtime.mean))**0.5
