...
class Model:
    def __init__(self, model_data):
        ...
        half2full = sum([site.nspinhalfs*[i] for i, site in enumerate(self.model_data.sites)], [])
        self.full2half = [[i for i, x in enumerate(half2full) if x == idx] for idx in range(self.Nfull)]
    ...
    def chirality_operators(self):
        ...
        def lift_tri(i, op):
            return sps.kron(sps.kron(sps.identity(8**i), op), sps.identity(8**(self.Nfull-i-1)))
        ...
        C = hamiltonian.heisen_bond(0,1,self.N) + j * hamiltonian.heisen_bond(1,2,self.N) + j**2 * hamiltonian.heisen_bond(2,0,self.N)

        ops['NematicityCorr'] = [C @ (hamiltonian.heisen_bond(3*i+0,3*i+1,self.N) +
                j * hamiltonian.heisen_bond(3*i+1,3*i+2,self.N) + j**2 * hamiltonian.heisen_bond(3*i+2,3*i+0,self.N)).H for i in range(0,self.Nfull)]
        return ops
        ...
        for x in range(self.model_data.Lx):
            for y in range(self.model_data.Ly):
                for uc in range(self.model_data.uc_site_count):
                    i = self.model_data.uc_site_count*(self.model_data.Lx*y+x)+uc
                    sign = sx**x * sy**y * (self.model_data.sites[i].sublattice_sign if suc < 0 else 1)
        ...
        for b in self.model_data.bonds:
            for spini, i in enumerate(self.full2half[b.i]):
                for spinj, j in enumerate(self.full2half[b.j]):
                    H += b.J[spini*len(self.full2half[b.j])+spinj] * hamiltonian.heisen_bond(i, j, self.N)
    ...
    def j_operators(self):
        ...
        for name, q in {'J':0, 'Jq1':2*np.pi/self.model_data.Lx, 'Jq2':4*np.pi/self.model_data.Lx}.items():
            ...
            for i, s in enumerate(self.model_data.sites):
                ...
                op += np.exp(1j*x*q)*sps.kron(sps.kron(sps.identity(2**pos), l_opers[s.nspinhalfs]),sps.identity(2**(self.N-pos-s.nspinhalfs)))
                pos += s.nspinhalfs
    ...
    def observables(self, params, Ts, E, psi):
        ...
        def j_obs():
            ...
            obs['JStruc1'] = ens.mean(np.real(Jq1)@np.real(Jq1)+np.imag(Jq1)@np.imag(Jq1))
            obs['JStruc2'] = ens.mean(np.real(Jq2)@np.real(Jq2)+np.imag(Jq2)@np.imag(Jq2))
            r = obs['JStruc1']/obs['JStruc2']
            obs['JCorrLen'] = self.model_data.Lx/2/np.pi*np.sqrt(np.maximum((r-1)/(4-r),0))
